...
export const AuthProvider = ({ children }) => {
  ...
  try {
    ...
    const updateProfile = async (userData, file) => {
        ...
        try
        {
            const response = await PostUpdateProfile(userData, file);
            data.status === 422 ? Toast.error(data.response?.data?.message) : Toast.success(data.message);
        ...
        {
            const message = ""
            const passError = error.response?.status === 422 ? error.response?.data?.message : "Failed to update Profile, please try again"
            passError ? setError(passError)  : setError(message)
