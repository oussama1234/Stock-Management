...
import { fetchUsers, addUser, deleteUser } from '../../Redux/UsersThunks';
...
const UsersPage = () => {
  ...
  const handleInputChange = (e) => {
    ...
    // i want to set formData with imagePreview if the user selects an image else it goes back to normal value
    ...
    //set form data with values if image set it to image, else set it to value
  ...
  const deleteUserFunction = async (userObj) => {
      ...
      const result = await dispatch(deleteUser({user: userObj}));
      if(deleteUser.fulfilled.match(result)){
        console.log("result deleted:", result);
        setUsers(users.filter(user => user.id !== userObj.id));
              ...
              <div className="p-6">
                ...
                <div className="flex justify-center space-x-3">
                  ...
                  <motion.button
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
                    onClick={() => deleteUserFunction(user.id)}
                    ...
                    <label className="relative cursor-pointer">
                      <div className="w-20 h-20 rounded-full bg-gradient-to-r from-blue-100 to-indigo-100 flex items-center justify-center border-4 border-white shadow-lg">
                        {formData.profileImage ||imagePreview ? (
                          <img 
                            src={formData.profileImage || imagePreview} 
                            alt="Preview"
                            className="w-full h-full object-cover rounded-full"
                          />
                        ) : (
