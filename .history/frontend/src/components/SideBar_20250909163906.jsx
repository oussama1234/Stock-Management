...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const menuItems = [
    { id: 1, name: 'Dashboard', icon: Home, link: DashboardRoute, hasSubmenu: false },
    { id: 2, name: 'Products', icon: Package, link: '#', hasSubmenu: true },
    { id: 3, name: 'Users', icon: Users, link: UsersRoute, hasSubmenu: false },
    { id: 4, name: 'Sales', icon: TrendingUp, link: '#', hasSubmenu: false },
    { id: 5, name: 'Purchases', icon: ShoppingCart, link: '#', hasSubmenu: false },
      ...
      >
        <div className="flex flex-col h-full">
              ...
              >
                <Link
                  ...
                  className={`flex items-center justify-between px-4 py-3 rounded-xl text-blue-100 hover:text-white hover:bg-indigo-700/40 transition-all duration-300 ${
                    isActive(item.link) ? 'bg-indigo-600 text-white shadow-md' : ''
                  }`}
                >
          ...
          <div className="p-4 border-t border-indigo-700/50">
            ...
            >
              <h3 className="text-white text-sm font-medium mb-1">Stock Manager v1.0</h3>
              <p className="text-blue-200 text-xs">Intelligent Inventory Management</p>
            </motion.div>
          </div>
        </div>
      </motion.div>
